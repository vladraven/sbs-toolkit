...
final class AnalyticsModule implements ModuleInterface {
	...
	public function boot(): void {
		...
		if ( ! is_admin() && SettingsManager::get( 'analytics', 'enabled', true ) ) {
	...
	public function inject_tracker_script(): void {
		...
		(function() {
			...
			let sessionId = sessionStorage.getItem('sbs_sid') || Math.random().toString(36).substring(2) + Date.now().toString(36);
			sessionStorage.setItem('sbs_sid', sessionId);

			function sendPing(timeSpent = 0, eventType = 'pageview', eventData = '') {
				let data = {
				...
				fetch('{$endpoint}', {
					...
					keepalive: true
				}).catch(err => console.error('SBS Tracker Error:', err));
			}
			...
			document.addEventListener('click', function(e) {
				...
				if (link && link.href) {
					...
					if (linkHost !== window.location.hostname && !link.href.startsWith('javascript:')) {
...
JS;
		...
		wp_register_script( 'sbs-tracker', '', [], false, true );
	...
	public function ajax_get_dashboard(): void {
		...
		$range   = isset( $_POST['range'] ) && $_POST['range'] === '30' ? 30 : 7;
		...
		// Исторические данные из stats
		$summary = $wpdb->get_row( $wpdb->prepare( "SELECT SUM(pageviews) as pv, SUM(unique_sessions) as us, AVG(avg_time_sec) as atime FROM {$stats_table} WHERE {$where_sql}", ...$args ) );
		
		// Плюсуем свежие сырые данные из raw
		$today_pv = $wpdb->get_var( "SELECT COUNT(*) FROM {$raw_table} WHERE is_bot = 0" );
		$today_us = $wpdb->get_var( "SELECT COUNT(DISTINCT session_id) FROM {$raw_table} WHERE is_bot = 0" );

		$total_pv = (int) ( $summary->pv ?? 0 ) + (int) $today_pv;
		$total_us = (int) ( $summary->us ?? 0 ) + (int) $today_us;
		$avg_time = round( (float) ( $summary->atime ?? 0 ), 1 );

		$referrers = $wpdb->get_results( $wpdb->prepare( "SELECT referrer_domain as label, SUM(pageviews) as pv FROM {$stats_table} WHERE {$where_sql} AND referrer_domain != '' GROUP BY referrer_domain ORDER BY pv DESC LIMIT 10", ...$args ) );
		$countries = $wpdb->get_results( $wpdb->prepare( "SELECT country as label, SUM(pageviews) as pv FROM {$stats_table} WHERE {$where_sql} GROUP BY country ORDER BY pv DESC LIMIT 10", ...$args ) );
		$oses      = $wpdb->get_results( $wpdb->prepare( "SELECT os as label, SUM(pageviews) as pv FROM {$stats_table} WHERE {$where_sql} GROUP BY os ORDER BY pv DESC LIMIT 10", ...$args ) );
		$browsers  = $wpdb->get_results( $wpdb->prepare( "SELECT browser as label, SUM(pageviews) as pv FROM {$stats_table} WHERE {$where_sql} GROUP BY browser ORDER BY pv DESC LIMIT 10", ...$args ) );
		$outbound  = $wpdb->get_results( $wpdb->prepare( "SELECT url_to as label, SUM(clicks) as pv FROM {$outbound_table} WHERE {$where_sql} GROUP BY url_to ORDER BY pv DESC LIMIT 10", ...$args ) );
